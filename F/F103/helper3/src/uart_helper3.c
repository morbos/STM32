...
u32
update_crc32(u32 crc_accum, char *data_blk_ptr, int data_blk_size)
{
    int i;
    int j;
    if(g_CRC32) {
...
{
    u8 crc8 = 0xff;
    putchar_no_waiting(c);
    update_crc8(crc8, c);
    putchar_no_waiting(crc8);
}
void
emit1_compute_crc8(u8 *crc8, u8 c)
{
    putchar_no_waiting(c);
    update_crc8(crc8, c);
}
...
//    uart_clear_fifos();
    ...
    u.m.len = scratch_val;
    u.m.off = g_fw_offset[mode];
    u.m.status = g_fw_status[mode];

    crc8 = 0xff;
    emit1_compute_crc8(&crc8, 0xa7);
    for(i=0; i < sizeof(u);i++) {
	emit1_compute_crc8(&crc8, u.c[i]);
    }
    putchar_no_waiting(crc8);
...
uart_check_start_ack(u32 mode, u32 *next_state)
{
	...
	*next_state = UART_START;
	...
	wr_ptr++;
	wr_ptr &= 0xff;
	acc = (wr_ptr - rd_ptr) & 0xff;
	if(acc >= want_len) {
	    if(buffer[rd_ptr] == 0x7a) {
		crc8 = 0xff;
		update_crc8(&crc8, buffer[rd_ptr]);
		rd_ptr++;
		rd_ptr &= 0xff;
		if(crc8 == buffer[rd_ptr]) {
		    first = 1;
		    *next_state = HDR_START;
		}
	    } else if(buffer[rd_ptr] == 0xdc) {
		crc8 = 0xff;
		update_crc8(&crc8, buffer[rd_ptr]);
		rd_ptr++;
		rd_ptr &= 0xff;
		...
		*next_state = g_helper_state[mode];
	    ...
	    // Don't move the rd_ptr. we need more data
	    *next_state = g_helper_state[mode];
	}
    }
}
...
req_bytes_polled(u32 mode, u32 want, u32 next_state, u32 *act_next_state)
{
    ...
    if(ptr) {
        crc = update_crc32(0, (char *)ptr, want);
...
check_uart_char(u32 mode, u32 pass, u32 fail, u32 restart)
{
	...
	if(acc >= want_len) {
		...
		rd_ptr &= 0xff;
		for(i=0;i < 4;i++) {
		    u.c[i] = buffer[rd_ptr + i];
		    update_crc8(&crc8, buffer[rd_ptr + i]);
		}
		// CRC8 == ? and expected offset?
		// Only if those two tests pass do we clear the status and proceed.
		crc_off = (rd_ptr + 4) & 0xff;
		if((buffer[crc_off] == crc8) && (u.x == g_fw_offset[mode])) {
		    g_fw_status[mode] &= ~UART_STATUS_MASK;
		    g_helper_state[mode] = fail;
		    first = 1;
		}
	    } else if(buffer[rd_ptr] == 0x7a) {
		crc8 = 0xff;
		update_crc8(&crc8, buffer[rd_ptr]);
		rd_ptr++;
		rd_ptr &= 0xff;
	    ...
	    } else if(acc >= 2) { // unhandled byte
		crc8 = 0xff;
		update_crc8(&crc8, buffer[rd_ptr]);
		rd_ptr++;
		rd_ptr &= 0xff;
		// If its unhandled, check its CRC8.
		// If clean, fail and resend len.
