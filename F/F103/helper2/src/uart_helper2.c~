...
enum {HDR_START=1,    HDR_START_ACK,          NEED_HDR,            CRC_FAIL_HDR,
      ...
      HELPER_IDLE,    UART_START,             UART_START_ACK
};
...
uart_poll(u32 mode, u32 *timeout)
{
    ...
    *timeout = 0;
	...
	switch(curr_state) {
	case NEED_HDR:
	    g_uart_status |= TO_HDR_DATA;
	    break;
	case NEED_BLOCK:
	    g_uart_status |= TO_BLOCK_DATA;
	    break;
	case NEED_CMD5:
	    g_uart_status |= TO_CMD5_DATA;
	    break;
	}
	log(g_uart_status | 0x80);
        *timeout = 1;
        first = 1;
        return 0;
    }
...
//    uart_clear_fifos();
    ...
    uart_write4(g_fw_offset);
    uart_write4(~g_fw_offset);
    uart_write2(g_uart_status);
    uart_write2(~g_uart_status);
...
check_uart_char(u32 mode, u32 pass, u32 fail, u32 restart)
{
	...
	switch(pass) {
	case NEED_HDR:
	    g_uart_status |= TO_HDR_START_ACK;
	    break;
	case NEED_BLOCK:
	    g_uart_status |= TO_BLOCK_LEN_ACK;
	    break;
	case NEED_CMD5:
	    g_uart_status |= TO_CMD5_LEN_ACK;
	    break;
	}
	log(g_uart_status | 0x80);
...
#endif
    if(getchar_no_waiting(&c) != FAIL) {
	if(g_uart_status & UART_STATUS_MASK) { // Is there an error?
	    buffer[pos] = c;
	    pos++;
	    if(pos == want_len) {
		if(buffer[0] == 0x6b) {
		    ...
		    coff = u.x;
		    if(off == ~coff) {
			g_uart_status &= ~UART_STATUS_MASK;
		    }
...
uart_helper()
{
    ...
    mode = HOST_UART;
    if(curr_state != g_helper_state[mode]) {
	curr_state = g_helper_state[mode] & 0xff;
	log(curr_state);
    }
    ...
    case CRC_FAIL_BLOCK:
	g_len[mode] |= 1;
	g_uart_status |= DATA_CRC_ERR;
	log(g_uart_status | 0x80);
	g_helper_state[mode] = HDR_PARSE;
	break;
    case CRC_FAIL_CMD5:
	g_len[mode] |= 1;
	g_uart_status |= CMD5_CRC_ERR;
	log(g_uart_status | 0x80);
	g_helper_state[mode] = HDR_PARSE;
	break;
    case BLOCK_COPY:
