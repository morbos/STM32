      ...
      if Check_Valid then
         ...
         if Compute_Aqi (Frm.Pm_Ug (PM2_5), Slot, AqiVal) then
            ...
            if First or Slot /= LastSlot then
               First := False;
               DrawRect (Display, 10, 10, 110, 136, AqA (Slot).Colour);
               DrawRect (Display, 11, 11, 108, 134, AqA (Slot).Colour);
               LastSlot := Slot;
            end if;
         end if;
         for J in Aq_Select loop
            if LastData (J) /= Gui (J).Val then
               X := 25;
               Y := UInt16 ((Aq_Select'Pos (J) + 1) * 18) + 5;
            ...
            begin
               if LastCo2Conc /= Curr_Co2_Reading.Concentration then
                  ...
                  DrawString (Display, X, Y,
                              "Co2 : "
                                &
                                Curr_Co2_Reading.Concentration'Image,
                              ST7735_BLUE,
                              ST7735_BLACK);
               ...
               if LastCo2Temp /= Curr_Co2_Reading.Temp then
                  ...
                  X := 25;
                  Y := UInt16 ((Idx + 1) * 18) + 5;
                  DrawString (Display, X, Y, "Temp : " & IT'Image, ST7735_BLUE, ST7735_BLACK);
