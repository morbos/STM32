...
package body SX1276_API is
   ...
   is
      ...
      --  An alias so we can test the RSSI ptr passed in
      A2 : System.Address;
      L2 : aliased Integer_Ptr := RSSI;
      for A2'Address use L2'Address;
   begin
      ...
      --  Now the Len's Integer_Ptr
      TTR := Test_Address (A);
      if TTR.Secure then
         return 0;
      end if;
      --  Now the RSSI's Integer_Ptr
      TTR := Test_Address (A2);
      if TTR.Secure then
         return 0;
      end if;
