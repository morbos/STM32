...
TX_THREAD       *thread_ptr;
    ...
    search_ptr = current_ptr =  pool_ptr -> tx_byte_pool_search;
    ...
    do
    {
        ...
        if (*((ULONG *) (current_ptr + sizeof(UCHAR *))) == TX_BYTE_BLOCK_FREE)
        {
            ...
            available_bytes =  next_ptr - current_ptr - sizeof(UCHAR *) - sizeof(ULONG);
            ...
            else
            {
                ...
                if (*((ULONG *) (next_ptr + sizeof(UCHAR *))) == TX_BYTE_BLOCK_FREE)
                {

                    /* Yes, neighbor block can be merged!  This is quickly accomplished
                       by updating the current block with the next blocks pointer.  */
                    *((UCHAR **) current_ptr) =  *((UCHAR **) next_ptr);
    ...
    if (available_bytes)
    {
        ...
        if ((available_bytes - memory_size) >= ((ULONG) TX_BYTE_BLOCK_MIN))
        {
            ...
            /* Split the block.  */
            next_ptr =  current_ptr + memory_size + sizeof(UCHAR *) + sizeof(ULONG);

            /* Setup the new free block.  */
            *((UCHAR **) next_ptr) =  *((UCHAR **) current_ptr);
            *((ULONG *) (next_ptr + sizeof(UCHAR *))) =  TX_BYTE_BLOCK_FREE;

            /* Increase the total fragment counter.  */
            pool_ptr -> tx_byte_pool_fragments++;

            /* Update the current pointer to point at the newly created block.  */
            *((UCHAR **) current_ptr) = next_ptr;
        ...
        *((TX_BYTE_POOL **) (current_ptr + sizeof(UCHAR *))) =  pool_ptr;

        /* Reduce the number of available bytes in the pool.  */
        pool_ptr -> tx_byte_pool_available =  pool_ptr -> tx_byte_pool_available - 
                                            available_bytes - sizeof(UCHAR *) - sizeof(ULONG);
        ...
        current_ptr =  current_ptr + sizeof(UCHAR *) + sizeof(ULONG);
    }
    else
    {
