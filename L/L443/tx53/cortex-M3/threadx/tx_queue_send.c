...
VOID            (*queue_send_notify)(struct TX_QUEUE_STRUCT *);
    ...
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_QUEUE_SEND, queue_ptr, source_ptr, wait_option, queue_ptr -> tx_queue_enqueued, TX_TRACE_QUEUE_EVENTS)

    /* Pickup the thread suspension count.  */
    suspended_count =  queue_ptr -> tx_queue_suspended_count;

    /* Determine if there is room in the queue.  */
    if ((queue_ptr -> tx_queue_available_storage) && (suspended_count == 0))
    {
    ...
    /* Determine if there is a thread waiting to receive a message.  */
    else if ((queue_ptr -> tx_queue_available_storage) && (suspended_count))
    {
