...
ULONG               ticks_left;              
    ...
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_TIMER_DEACTIVATE, timer_ptr, &ticks_left, 0, 0, TX_TRACE_TIMER_EVENTS)
        ...
        (list_head < _tx_timer_list_end))
    {
        ...
        if (list_head >= _tx_timer_current_ptr)
        {
            ...
            ticks_left =  (list_head - _tx_timer_current_ptr) + 1;
        }
        else
        {

            /* Calculate the ticks left with a wrapped list condition.  */
            ticks_left =  (list_head - _tx_timer_list_start);
