...
package body SX1276 is
   ...
   function IsIdleMode (This : in out SX1276_Radio) return Boolean
   is
      As_Byte : UInt8;
      Reg     : OpMode_Register;
      for As_Byte'Address use Reg'Address;
   begin
      Read (This.Port, SX1276_OP_MODE, As_Byte);
      if Reg.Mode = STDBY then
         return True;
      else
         return False;
      end if;
...
--    CurrentMode := RXSINGLE;
   ...
   procedure ResetFifoAddr (This : in out SX1276_Radio)
   is
   begin
   ...
   begin
      ...
      loop
         Read (This.Port, SX1276_FIFO_RX_CURRENT_ADDR, Value => X);
         exit when X = 0;
      end loop;
      ...
      Write (This.Port, SX1276_PAYLOAD_LENGTH, Len);
...
--      This.SetRxMode;
   ...
   begin
      ...
      if Exti.Rx_Done_Flag then
         ...
         Write (This.Port, SX1276_FIFO_ADDR_PTR, 0);
         declare
            LBuffer : SPI_Data_8b (1 .. Integer (Len));
         begin
            Read_Buffer (This.Port, SX1276_FIFO, LBuffer);
            for I in LBuffer'Range loop
               Buffer (I) := LBuffer (I);
            end loop;
         end;
         Read (This.Port, SX1276_PKT_RSSI_VALUE, RSSI);
         Result := True;
      end if;
...
--      This.SetIdleMode;
   ...
   procedure Recv_Non_Block (This   : in out SX1276_Radio;
                             Buffer : out SPI_Data_8b;
                             Len    : out UInt8;
                             RSSI   : out UInt8)
   is
