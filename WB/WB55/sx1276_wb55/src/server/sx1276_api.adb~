...
package body SX1276_API is
   ...
   procedure Recv_Non_Block_SX1276 (Buffer  : out LoRa_Packet;
                                    Len     : Integer_Ptr;
                                    RSSI    : Integer_Ptr)
   is
      LocalLen  : aliased UInt8;
      ...
      LBuffer  : SPI_Data_8b (1 .. 255);
   begin
      SX.Recv_Non_Block (LBuffer, LocalLen, LocalRSSI);
      for I in 1 .. LocalLen loop
         Buffer (Integer (I)) := LBuffer (Integer (I));
      end loop;
      Len.all := Integer (LocalLen);
      RSSI.all := Integer (LocalRSSI);
   end Recv_Non_Block_SX1276;

   function Is_Idle_SX1276 return Boolean
   is
   begin
      return SX.IsIdleMode;
   end Is_Idle_SX1276;
